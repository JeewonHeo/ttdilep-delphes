...
int main(int argc, char* argv[]) {
  if (argc != 6) {
    // FIXME
    std::cerr << "Usage: analyseTTDilepton isTT chargeAlgorithm exponent outpath inputFile" << std::endl;
    return 1;
  }
  ...
  TTDileptonAnalyser analyser(in_path, out_path, is_tt, chargeAlgorithm, exponent);
  analyser.loop();
