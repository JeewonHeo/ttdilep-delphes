...
TTSemileptonAnalyser::TTSemileptonAnalyser(const TString & in_path,
...
TTSemileptonAnalyser::~TTSemileptonAnalyser() {
...
void TTSemileptonAnalyser::makeBranch(TTree* tree, bool is_matched) {
  resetBranch();
...
void TTSemileptonAnalyser::resetBranch() {
  b_label_ = -1;
  b_ch_ = -1;
//  b_offline_ = false;
...
void TTSemileptonAnalyser::bookHistograms() {
...
void TTSemileptonAnalyser::resetMemberData() {
...
std::vector<const GenParticle*> TTSemileptonAnalyser::getDaughters(const GenParticle* mother) {
...
const GenParticle* TTSemileptonAnalyser::getLast(const GenParticle* particle) {
...
TTSemileptonAnalyser::getTopDecayProducts(const GenParticle* top_quark) {
  ...
  if (((std::abs(d0->PID) == 5) or (std::abs(d0->PID) == 3)) and (std::abs(d1->PID) == 24)) { quark = d0;
    ...
    auto msg = Form("[ERROR] TTSemileptonAnalyser::getTopDecayProducts >>> (PID=%d, Status=%d) --> (%d, %d), (%d, %d)",
...
bool TTSemileptonAnalyser::isQuark(int pid) {
...
std::pair<const GenParticle*, const GenParticle*>
TTSemileptonAnalyser::getWBosonDecayProducts(const GenParticle* w_boson) {
...
//  } else {
//    auto msg = Form("[ERROR] TTSemileptonAnalyser::getWBosonDecayProducts >>> (PID=%d, Status=%d) --> (%d, %d), (%d, %d)",
...
Bool_t TTSemileptonAnalyser::matchPartonsWithJets() {
  ...
  // matchingJets
  std::vector<const GenParticle*> partons({wq1_, wq2_});
...
bool TTSemileptonAnalyser::performTotalMinDist(
  ...
  for (auto&& [parton_index, jet_index] : match) {
    // std::cout << parton_index << " " << jet_index << std::endl;
    if (jet_index < 0) {
      is_matched = false;
      break;
    }
...
Bool_t TTSemileptonAnalyser::selectEvent() {
  /*
  // initial
  cutflow_->Fill(1);
  ...
  */
  ...
  UInt_t num_selected_electrons = selected_electrons_.size();
  if (num_selected_muons + num_selected_electrons != 1) return false;

  if (num_selected_muons) {
    lep_p4_.push_back(selected_muons_[0]->P4());
  } else {
    lep_p4_.push_back(selected_electrons_[0]->P4());
  }
  ...
  auto missing_et = getMissingET(0);
  b_met_ = missing_et->MET;
  if (b_met_ < kMinMET_) return false;
...
Int_t TTSemileptonAnalyser::getPFIndex(Int_t pid, Int_t charge) {
...
TTSemileptonAnalyser::getJetPartonMatchingCode(const GenParticle* parton) {
  Int_t code1, code2;
  if (parton == wq1_) {
    code1 = parton->PID;
  } else if (parton == wq2_) {
...
void TTSemileptonAnalyser::HadronReconstruction() {
...
void TTSemileptonAnalyser::analyseJet() {
  Int_t jet_count = 0;
...
void TTSemileptonAnalyser::analyseLepton() {
...
void TTSemileptonAnalyser::analyse() {
  // analyseLepton();
  analyseJet();
...
void TTSemileptonAnalyser::loop() {
